...
- **Productivity score**: Overall effectiveness rating based on session outcomes
- **Actionable recommendations**: Specific tips to reduce friction and improve results
- **Friction log**: Detailed descriptions of what went wrong in each session
- **Helpfulness → Success correlation**: See how Claude's helpfulness level impacts outcomes
- **Activity breakdown**: What you work on, session types, and goal categories
- **Powered by `/insights`**: Reads facet data generated by Claude Code's `/insights` command
...
### Dashboard - Insights
![Insights Tab](https://www.rshankar.com/assets/images/claudit/dashboard-insights.png)
...
4. **Insight facets** (`~/.claude/usage-data/facets/*.json`) - Session analysis from `/insights`
5. **macOS Keychain** - OAuth credentials (read-only)
...
Open the Dashboard for detailed analytics with 5 tabs:
...
| **Insights** | Productivity score, friction log, recommendations, activity & quality metrics |
